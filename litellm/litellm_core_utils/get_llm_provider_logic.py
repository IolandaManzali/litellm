        ...
        if api_base:
            for endpoint in litellm.openai_compatible_endpoints:
                if endpoint in api_base:
                    ...
                    elif endpoint == "open.bigmodel.cn/api/paas/v4":
                        custom_llm_provider = "zhipu"
                        dynamic_api_key = get_secret_str("ZHIPU_API_KEY")
...
def _get_openai_compatible_provider_info(  # noqa: PLR0915
    ...
    elif custom_llm_provider == "zhipu":
        api_base = (
            api_base
            or get_secret("ZHIPU_API_BASE")
            or "https://open.bigmodel.cn/api/paas/v4"
        )  # type: ignore
        dynamic_api_key = (
            api_key
            or get_secret_str("ZHIPU_API_KEY") 
        )
    if api_base is not None and not isinstance(api_base, str):
